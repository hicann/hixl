# 接口参考（C++）

- [头文件和库文件说明](header_files_and_library_files.md)
- [简介](brife.md)
- [LLM-DataDist接口](LLM-DataDist-interface.md)
- [LLM-DataDist数据结构](LLM-DataDist-data-structure.md)
- [LLM-DataDist错误码](LLM-DataDist-error-code.md)
- [HIXL_CS接口](HIXL_CS-interface.md)
- [HIXL接口](HIXL-interface.md)
- [HIXL数据结构](HIXL-data-structure.md)
- [HIXL错误码](HIXL-error-code.md)
- [不支持的接口](unsupported-interface.md)
- [待废弃_ADXL接口](deprecated_ADXL-interface.md)
- [待废弃_ADXL数据结构](deprecated_ADXL-data-structure.md)
- [待废弃_ADXL错误码](deprecated_ADXL-error-code.md)
