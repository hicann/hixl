...
以下场景如果使用了安全函数且 destMax 未超出最大限制时，可以不进行返回值检查：

| 编号 | 场景 | 说明 |
|------|------|------|
| 1 | 对固定长度的数组进行初始化，或对固定长度的结构体进行内存初始化 | 如 `memset_s(&addr, sizeof(addr), 0, sizeof(addr))`，destMax == count == sizeof(结构体/数组) |
| 2 | 函数参数中有指向内存的指针参数和对应的内存大小的参数时，对该内存进行初始化 | 调用方负责保证指针有效性和大小匹配 |
| 3 | 从堆中分配内存后，赋予初值 | 如 `ptr = malloc(n); memset_s(ptr, n, 0, n);` |
| 4 | 根据源内存的大小进行同等大小的内存复制 | 如 `memcpy_s(&dst, sizeof(dst), src, sizeof(dst))`，destMax == count == sizeof(目标) |
| 5 | 源缓冲区全部是静态字符/字符串常量且目的缓冲区总大小能完全容纳源缓冲区中的数据及 null 结束符 | 如 `snprintf_s(buf, sizeof(buf), sizeof(buf) - 1, "ID:0x%x", val);`，格式串为字面量且最大输出可静态证明不超过 destMax |

**例外 2：在安全函数返回值检查的错误处理代码中，如果又调用了安全函数，可以不进行返回值检查。**
...
为了最大程度上在ABI层面的兼容，对外结构体接口添加新字段时必须在结构体最后添加。完整的ABI兼容性规则（STL类型准入、reserved空间使用、枚举值演进、符号演进等）见[ABI兼容性编码规范](cpp-abi.md)。

#### 10.12 外部接口或数据结构变更必须考虑兼容性

外部接口、接口参数、返回值、数据结构、消息字段等变更会引起版本兼容性问题，非必要不建议变更。ABI变更的判定清单与评审要求见[ABI兼容性编码规范](cpp-abi.md)。
