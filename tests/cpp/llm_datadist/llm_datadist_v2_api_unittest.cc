...
// Helper struct to hold common test context for kv cache setup + link + pull preparation
struct KvCacheTestContext {
  KvCacheSetupResult d_setup;
  KvCacheSetupResult p_setup;
  ClusterInfo cluster_info;
  CacheIndex cache_index;
  Cache dst_cache;
};

// Helper to set up kv caches on both sides, link clusters, and prepare pull cache structures.
// This extracts the common logic shared by TestLocalCommResA2 and TestLocalCommResA3.
inline KvCacheTestContext SetupKvCachesAndLink(LlmDataDist &llm_datadist_d, LlmDataDist &llm_datadist_p,
                                               const CacheDesc &kv_desc) {
  KvCacheTestContext ctx;
  ctx.d_setup = SetupKvCache(llm_datadist_d, kv_desc);
  ctx.p_setup = SetupKvCache(llm_datadist_p, kv_desc);

  // link
  IpInfo ip_info;
  ip_info.ip = "127.0.0.1";
  ip_info.port = 26000;
  ctx.cluster_info.local_ip_infos = {ip_info};
  ctx.cluster_info.remote_ip_infos = {ip_info};
  std::vector<ge::Status> rets;
  EXPECT_EQ(llm_datadist_d.LinkLlmClusters({ctx.cluster_info}, rets), ge::SUCCESS);

  // pull cache setup
  ctx.cache_index = CacheIndex{1U, ctx.p_setup.cache_id, 0U, {}};
  ctx.dst_cache = Cache{};
  ctx.dst_cache.cache_id = ctx.d_setup.cache_id;
...
TEST_F(LlmDataDistUTest, TestLocalCommResA2) {
  ...
  // pull cache
  EXPECT_EQ(llm_datadist_d.PullKvCache(ctx.cache_index, ctx.dst_cache), ge::SUCCESS);

  // push cache
  CacheIndex dst_cache_index{2U, ctx.d_setup.cache_id, 0U, {}};
  Cache src_cache{};
  src_cache.cache_id = ctx.p_setup.cache_id;
  KvCacheExtParam ext_param{{0, 0}, {0, 0}, 2, {}};
...
TEST_F(LlmDataDistUTest, TestLocalCommResA3) {
  LlmDataDist llm_datadist_p(1U, LlmRole::kPrompt);
  LlmDataDist llm_datadist_d(2U, LlmRole::kDecoder);
  InitTestLlmDataDistV12(llm_datadist_p, "0", "1.1.1.1", true);
  InitTestLlmDataDistV12(llm_datadist_d, "1", "1.1.1.2", false);
  ...
  // pull cache
  EXPECT_EQ(llm_datadist_d.PullKvBlocks(ctx.cache_index, ctx.dst_cache, {0}, {0}), ge::SUCCESS);
...
TEST_F(LlmDataDistUTest, TestVersionOnlyLocalCommResA2) {
  class AutoCommResV1RuntimeMock : public llm::AutoCommResRuntimeMock {
   public:
    const char *aclrtGetSocName() override {
      return "Ascend910B1";
  ...
  options_p[llm_datadist::OPTION_LISTEN_IP_INFO] = "127.0.0.1:26000";
  options_p[llm_datadist::OPTION_DEVICE_ID] = "0";
  options_p["llm.LocalCommRes"] = R"({"version": "1.0"})";
  ...
  std::map<AscendString, AscendString> options_d;
  options_d[llm_datadist::OPTION_DEVICE_ID] = "1";
  options_d["llm.LocalCommRes"] = R"({"version": "1.0"})";
...
TEST_F(LlmDataDistUTest, TestVersionOnlyLocalCommResA3) {
  LlmDataDist llm_datadist_p(1U, LlmRole::kPrompt);
  std::map<AscendString, AscendString> options_p;
  options_p[llm_datadist::OPTION_LISTEN_IP_INFO] = "127.0.0.1:26000";
  options_p[llm_datadist::OPTION_DEVICE_ID] = "0";
  options_p["llm.LocalCommRes"] = R"({"version": "1.2"})";
  ...
  std::map<AscendString, AscendString> options_d;
  options_d[llm_datadist::OPTION_DEVICE_ID] = "1";
  options_d["llm.LocalCommRes"] = R"({"version": "1.2"})";
...
TEST_F(LlmDataDistUTest, TestPushWithRangeAndTensorNumV2) {
  ...
  TestPush({{0, 1}, {0, 1}, 7, {}}, false);
  TestPush({{0, 1}, {0, 1}, 17, {}}, false);
  // test invalid range and tensor num
  TestPush({{-2, 0}, {0, 1}, 2, {}}, false);
  TestPush({{0, 1}, {0, 3}, 2, {}}, false);
  TestPush({{4, 5}, {0, 1}, 2, {}}, false);
  TestPush({{0, 1}, {4, 5}, 2, {}}, false);
...
TEST_F(LlmDataDistUTest, TestPullWithRangeAndTensorNumV2) {
  ...
  TestPull({{0, 1}, {0, 1}, 2, {}}, true);
  TestPull({{0, 1}, {0, 1}, 7, {}}, false);
  TestPull({{0, 1}, {0, 1}, 17, {}}, false);
  TestPull({{0, 1}, {0, 2}, 2, {}}, false);
