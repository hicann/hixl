...
class HixlUTest : public ::testing::Test {
 protected:
  ...
  void RunMultiThreadTransferAsyncTest() {
    Hixl engine1;
    Hixl engine2;
    ...
    DispatchAsyncWrites(engine1, desc, req_list, kThreadCount);
    test_helpers::WaitForAllAsyncTransfers(engine1, req_list, kThreadCount);
...
TEST_F(HixlUTest, TestHixlTransferAsync) {
  ...
  ASSERT_EQ(engine1.TransferAsync("127.0.0.1:26201", READ, {desc}, {}, req), SUCCESS);
  ...
  src = 1;
  ASSERT_EQ(engine1.TransferAsync("127.0.0.1:26201", WRITE, {desc}, {}, req), SUCCESS);
...
TEST_F(HixlUTest, TestHixlTransferAsyncWithMultiThread) {
  RunMultiThreadTransferAsyncTest();
}
...
TEST_F(HixlUTest, TestHixlGetTransferStatusWithInterrupt) {
  ...
  engine1.Disconnect("127.0.0.1:26201");
  TransferStatus status = TransferStatus::WAITING;
...
TEST_F(HixlUTest, TestHixlMultiSendNotifies) {
  ...
  EXPECT_EQ(engine3.Initialize("127.0.0.1:26202", options3), SUCCESS);
  ...
  notifies.clear();
  EXPECT_EQ(engine1.Disconnect("127.0.0.1:26201"), SUCCESS);
  EXPECT_EQ(engine3.Disconnect("127.0.0.1:26201"), SUCCESS);
  engine1.Finalize();
  engine2.Finalize();
  engine3.Finalize();
}
...
TEST_F(HixlUTest, TestHixlSendNotifyTimeout) {
  ...
  for (int i = 0; i < 5; ++i) {
    ...
    EXPECT_EQ(engine1.SendNotify("127.0.0.1:26201", notify, 1), TIMEOUT);
  }
  // sleep 100 ms then get notifies
  std::this_thread::sleep_for(std::chrono::milliseconds(100));
...
TEST_F(HixlUTest, TestCommEngineGetTransferStatusUnsupported) {
  ...
  Hixl engine;
  engine.Initialize("127.0.0.1:26200", options);
