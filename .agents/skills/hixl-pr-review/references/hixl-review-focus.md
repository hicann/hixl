...
| 11 | 对外接口文档一致性 | 检查 `include/` 下公开头文件变更时，`docs/zh/api/cpp/` 中是否有对应的接口说明文档且内容正确。具体包括：头文件变更时对应 `docs/zh/api/cpp/` 文档是否同步更新；文档中的函数原型与头文件签名是否严格一致（返回类型、参数类型、参数名、默认值、const 引用）；每个公开 API 是否有完整的 `@brief`/`@param`/`@return` Doxygen 注释且参数名与原型一致；接口说明文档章节是否齐全（产品支持情况、功能说明、函数原型、参数说明、返回值说明、约束说明），依据 api-doc-generator 规则 #1-#5。 |
