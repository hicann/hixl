...
Status HixlEngine::Connect(const AscendString &remote_engine, int32_t timeout_in_millis) {
  ...
  HIXL_DISMISSABLE_GUARD(
      connect_guard, ([this, is_new_client, remote_engine_str = std::string(remote_engine.GetString()), &client_ptr]() {
...
Status HixlEngine::AutoConnect(const AscendString &remote_engine, int32_t timeout_in_millis) {
  ...
  HIXL_DISMISSABLE_GUARD(connect_guard, ([this, remote_engine_str = std::string(remote_engine.GetString()),
                                          &client_ptr]() { (void)client_manager_.DestroyClient(remote_engine_str); }));
