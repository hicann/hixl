...
ge::Status ParseMemoryPoolConfig(const std::string &mem_pool_config, size_t &pool_size, size_t &page_shift) {
  ...
  try {
    ...
    if (json_obj.contains("page_shift")) {
      ...
      // page_shift valid range is [10, 31)
      LLM_CHK_BOOL_RET_STATUS((page_shift >= 10) && (page_shift < 31), ge::LLM_PARAM_INVALID,
...
ge::Status ResolveSwapNumBlocks(int64_t cache_id, CacheManager *cache_manager, uint64_t &num_blocks) {
  if (cache_id < 0) {
    // Address-only swap path (Python swap_blocks passes cache_id=-1); block indices are
    // validated in Python and copy ranges are checked in SwapImpl.
    num_blocks = 0U;
    return ge::SUCCESS;
  }
  CacheEntry cache_entry;
  LLM_CHK_BOOL_RET_STATUS(cache_manager->GetCacheEntry(cache_id, cache_entry), ge::LLM_KV_CACHE_NOT_EXIST,
                          "cache id:%ld not found", cache_id);
  num_blocks = cache_entry.num_blocks > 0U ? cache_entry.num_blocks : static_cast<uint64_t>(cache_entry.batch_size);
  LLM_CHK_BOOL_RET_STATUS(num_blocks > 0U, ge::LLM_PARAM_INVALID, "cache id:%ld has no blocks", cache_id);
  return ge::SUCCESS;
}
}  // namespace
ge::Status DataCacheEngine::Register(const llm::CacheDesc &cache_desc, const std::vector<CacheKey> &cache_keys,
  ...
  LLM_CHK_STATUS_RET(LLMUtils::CalcTensorMemSize(cache_desc.shape, cache_desc.data_type, tensor_size),
                     "Failed to calc tensor size, shape = %s, dtype = %d", hixl::ToString(cache_desc.shape).c_str(),
                     static_cast<int32_t>(cache_desc.data_type));
  LLMLOGI("[Register] start, placement:%u", static_cast<uint32_t>(cache_desc.placement));
  LLM_CHK_STATUS_RET(
      comm_mem_manager_->RegisterCacheMem(cache_id, cache_desc, cache.per_device_tensor_addrs[0U], tensor_size),
...
ge::Status DataCacheEngine::Unregister(int64_t cache_id) {
  ...
  std::lock_guard<std::mutex> lock(mu_);
  LLM_CHK_STATUS_RET(comm_mem_manager_->UnregisterCacheMem(cache_id), "Unregister cache addr failed, cache_id = %ld.",
                     cache_id);
  LLM_CHK_STATUS_RET(cache_manager_->UnregisterCacheEntry(cache_id), "Unregister cache entry failed, cache_id = %ld.",
                     cache_id);
  LLMLOGI("[cache_id:%ld][Unregister] success, cost = %ld ms", cache_id,
...
ge::Status DataCacheEngine::PullCache(int64_t cache_id, const CacheKey &cache_key,
  ...
  if (access_remote_cache_) {
    ...
    LLMLOGI(
        "[PullCache] success, cache_id = %ld, num_tensors = %zu, stride = %lu, "
        "pull_size = %ld, local_block_cnt = %zu, remote_block_cnt = %zu",
        cache_id, cache_entry.cache_addrs.size(), cache_entry.stride, pull_cache_param.size,
        pull_cache_param.decoder_blocks.size(), pull_cache_param.prompt_blocks.size());
    return ge::SUCCESS;
  }
  LLM_DISMISSABLE_GUARD(abort_stream, [this]() -> void { LLM_CHK_ACL(aclrtStreamAbort(req_stream_)); });
...
ge::Status DataCacheEngine::SwapBlocks(const Cache &src, const Cache &dst, const uint64_t block_size,
  ...
  const bool src_by_addr = src.cache_id < 0;
  const bool dst_by_addr = dst.cache_id < 0;
  LLM_CHK_BOOL_RET_STATUS(src_by_addr == dst_by_addr, ge::LLM_PARAM_INVALID,
                          "src/dst cache_id must both be registered(>=0) or both be address-only(-1), "
                          "got src:%ld dst:%ld",
                          src.cache_id, dst.cache_id);

  uint64_t src_num_blocks = 0U;
  uint64_t dst_num_blocks = 0U;
  LLM_CHK_STATUS_RET(ResolveSwapNumBlocks(src.cache_id, cache_manager_, src_num_blocks),
                     "resolve src num blocks failed");
  LLM_CHK_STATUS_RET(ResolveSwapNumBlocks(dst.cache_id, cache_manager_, dst_num_blocks),
                     "resolve dst num blocks failed");

  SwapImpl swap_impl(device_id_);
  LLM_CHK_STATUS_RET(swap_impl.SwapBlocksV2(src, dst, block_size, type, block_mapping, src_num_blocks, dst_num_blocks));
  return ge::SUCCESS;
}
...
ge::Status DataCacheEngine::Initialize(const std::map<ge::AscendString, ge::AscendString> &options) {
  ...
  LLM_CHK_STATUS_RET(
      GlobalMemManager::GetInstance().RegisterMem(buffer_and_size.first, buffer_and_size.second,
  ...
  // create stream
  LLM_ASSERT_RT_OK(aclrtCreateStreamWithConfig(&req_stream_, 0, ACL_STREAM_FAST_LAUNCH | ACL_STREAM_FAST_SYNC));
...
ge::Status DataCacheEngine::InitializeDeviceMemoryPool(const std::map<ge::AscendString, ge::AscendString> &options) {
  ...
  LLM_CHK_STATUS_RET(GlobalMemManager::GetInstance().RegisterMem(npu_pool_memory_, npu_pool_size_,
                                                                 CommMemType::COMM_MEM_TYPE_DEVICE, npu_pool_handle_));
...
ge::Status DataCacheEngine::Allocate(const CacheDesc &cache_desc, const std::vector<CacheKey> &cache_keys,
                                     Cache &cache) {
  LLM_CHK_BOOL_RET_STATUS(((npu_pool_memory_ != nullptr) || (host_pool_memory_ != nullptr)),
                          ge::LLM_FEATURE_NOT_ENABLED, "memory pool is not enabled");
  LLM_CHK_BOOL_RET_STATUS(
...
ge::Status DataCacheEngine::CheckParam(const CacheEntry &cache_entry, const PullCacheParam &pull_cache_param) {
  ...
  if (pull_cache_param.decoder_blocks.empty()) {
    LLM_CHK_BOOL_RET_STATUS(cache_entry.num_blocks == 0 || cache_entry.cache_mem_type == CacheMemType::MIX,
                            ge::LLM_PARAM_INVALID, "check failed, request expect local cache is non-blocks");
    LLM_CHK_BOOL_RET_STATUS(
        (pull_cache_param.size < 0) || (static_cast<uint64_t>(pull_cache_param.size) <= cache_entry.stride),
        ge::LLM_PARAM_INVALID, "pull_size(%ld) > cache stride(%lu)", pull_cache_param.size, cache_entry.stride);
  } else {
  ...
  if ((cache_entry.placement == CachePlacement::HOST) && (cache_entry.cache_mem_type == CacheMemType::BLOCKS)) {
    for (const auto block_index : pull_cache_param.decoder_blocks) {
      LLM_CHK_BOOL_RET_STATUS(block_index < cache_entry.num_blocks, ge::LLM_PARAM_INVALID,
...
ge::Status DataCacheEngine::CheckTensorIndices(const CacheEntry &cache_entry, const PullCacheParam &pull_cache_param) {
  ...
  if ((!pull_cache_param.src_tensor_indices.empty()) || (!pull_cache_param.dst_tensor_indices.empty())) {
    LLM_CHK_BOOL_RET_STATUS(remainder == 0U, ge::LLM_PARAM_INVALID,
                            "When using layer wise transfer, the tensor_num [%zu] of caches must be a multiple of "
                            "tensor_num_per_layer[%lu].",
...
ge::Status DataCacheEngine::TransferCache(const uint64_t task_id, const TransferCacheConfig &transfer_cache_config,
  ...
  std::call_once(create_stream_once_flag_, [&ret, this]() {
    ret = aclrtCreateStreamWithConfig(&transfer_stream_, 0, ACL_STREAM_FAST_LAUNCH | ACL_STREAM_FAST_SYNC);
  });
  LLM_ASSERT_RT_OK(ret, "create transfer stream failed");
  LLM_ASSERT_NOTNULL(transfer_stream_, "transfer stream is nullptr");
  LLM_DISMISSABLE_GUARD(abort_stream, [this]() -> void { LLM_CHK_ACL(aclrtStreamAbort(transfer_stream_)); });
