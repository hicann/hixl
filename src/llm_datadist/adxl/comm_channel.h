...
class CommChannel {
 ...
 private:
  ...
  void CompleteRequest(uint64_t id, const std::chrono::steady_clock::time_point &transfer_start,
  ...
  // Single link-level lock: serializes submit, sync, async poll, slot abort, and disconnect teardown so
