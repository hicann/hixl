...
bool IsReadTransferType(TransferType type) {
...
Status BufferTransferService::Transfer(const ChannelPtr &channel, TransferType type,
  ...
  time_cost = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - start).count();
  StatisticManager::GetInstance().UpdateBufferTransferCost(
      channel->GetStatisticChannelId(), time_cost, GetTransferBytes(op_descs), GetTransferOpDescCount(op_descs));
  return SUCCESS;
}
...
Status BufferTransferService::FlushBatch(const ChannelPtr &channel, const std::vector<TransferOpDesc> &op_descs,
  ...
  if (type == TransferType::kReadRH2H || type == TransferType::kReadRD2H || type == TransferType::kReadRH2D ||
    ...
    auto kind = (type == TransferType::kWriteD2RH || type == TransferType::kWriteD2RD) ? ACL_MEMCPY_DEVICE_TO_DEVICE
...
Status BufferTransferService::HandleBufferD2D(const ChannelPtr &channel, BufferReq &buffer_req) {
  ...
  ADXL_CHK_STATUS_RET(D2DTransfer(channel, op, op_descs, left_time), "D2D transfer failed.");
  time_cost = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - start).count();
...
Status BufferTransferService::D2DTransfer(const ChannelPtr &channel, TransferOp transfer_op,
  ...
  auto issue_fn = [&channel, transfer_op, &op_descs, timeout](aclrtStream stream) -> Status {
    return channel->TransferAsyncWithTimeout(transfer_op, op_descs, stream, timeout);
  };
  ADXL_CHK_STATUS_RET(channel->RunSyncOnSlot(issue_fn, static_cast<int32_t>(timeout_in_millis)), "transfer failed.");
  return SUCCESS;
}
...
Status BufferTransferService::BuildBufferSliceAddrs(uintptr_t base_addr, const std::vector<size_t> &buffer_lens,
  ...
  for (size_t i = 0U; i < count; ++i) {
    ...
    ADXL_CHK_BOOL_RET_STATUS(!ge::AddOverflow(base_addr, accumulated, slice_addr), PARAM_INVALID,
                             "base_addr + accumulated overflow at index:%zu.", i);
    addrs.emplace_back(slice_addr);
    ADXL_CHK_BOOL_RET_STATUS(
        !ge::AddOverflow(accumulated, buffer_lens[i], accumulated) && (accumulated <= max_total_len), PARAM_INVALID,
        "accumulated buffer length out of range at index:%zu, max_total_len:%lu.", i, max_total_len);
  }
  return SUCCESS;
}
...
Status BufferTransferService::ProcessCopyWithBatch(const SliceInfo &slice_info, uint64_t batch_num,
  ...
  for (uint64_t i = 0; i < batch_num; i++) {
    auto loc = aclrtMemLocation{static_cast<uint32_t>(device_id_), aclrtMemLocationType::ACL_MEM_LOCATION_TYPE_DEVICE};
    auto another_loc = aclrtMemLocation{0, aclrtMemLocationType::ACL_MEM_LOCATION_TYPE_HOST};
    if (kind == ACL_MEMCPY_DEVICE_TO_HOST) {
      attrs[i] = aclrtMemcpyBatchAttr{another_loc, loc, {}};
    } else {
      attrs[i] = aclrtMemcpyBatchAttr{loc, another_loc, {}};
    }
    attrsIds[i] = idx++;
...
Status BufferTransferService::ProcessCopyWithAsync(const ChannelPtr &channel, const std::vector<uintptr_t> &src_addrs,
                                                   ...
                                                   CopyExtraInfo extra_info) {
  auto timeout = extra_info.second;
  auto memcpy_kind = extra_info.first;
  auto start = std::chrono::steady_clock::now();
  // extra_info.second carries the copy budget in microseconds; RunSyncOnSlot expects milliseconds. The memcpy issue
  // runs inside RunSyncOnSlot's lambda, so nothing is spent here before the sync — convert the full budget directly.
  auto timeout_in_millis = timeout / kMillisToMicros;
  ADXL_CHK_BOOL_RET_STATUS(timeout_in_millis > 0, TIMEOUT, "Transfer timeout.");
  auto issue_fn = [&src_addrs, &dst_addrs, &sizes, memcpy_kind](aclrtStream stream) -> Status {
    for (size_t i = 0; i < src_addrs.size(); ++i) {
      ADXL_CHK_ACL_RET(aclrtMemcpyAsync(llm::ValueToPtr(dst_addrs[i]), sizes[i], llm::ValueToPtr(src_addrs[i]),
                                        sizes[i], memcpy_kind, stream));
    }
    return SUCCESS;
  };
  ADXL_CHK_STATUS_RET(channel->RunSyncOnSlot(issue_fn, static_cast<int32_t>(timeout_in_millis)), "Async copy failed.");
...
Status BufferTransferService::ProcessBufferReadCopyResp(const ChannelPtr &channel, BufferResp &buffer_resp) {
  auto start = std::chrono::steady_clock::now();
  ADXL_CHK_BOOL_RET_STATUS(buffer_resp.src_addrs.size() == buffer_resp.buffer_lens.size(), FAILED, "Addr not valid.");
  uint64_t max_total_len = 0U;
  ADXL_CHK_STATUS_RET(ResolveReadCopyTotalLen(buffer_resp, max_total_len), "Failed to resolve read copy total length.");
  ADXL_CHK_BOOL_RET_STATUS(max_total_len <= buffer_size_, PARAM_INVALID,
                           "max_total_len:%lu exceeds local buffer_size_:%lu.", max_total_len, buffer_size_);
  std::vector<uintptr_t> buffer_addrs;
...
Status BufferTransferService::HandleBufferResp(const ChannelPtr &channel, BufferResp &buffer_resp) {
  ...
  const auto &buf_set = req_it->second;
  ...
  const bool is_read_with_src = IsReadTransferType(buffer_resp.transfer_type) && !buffer_resp.src_addrs.empty();
  if (is_read_with_src) {
    ADXL_CHK_STATUS_RET(ProcessBufferReadCopyResp(channel, buffer_resp), "Failed to process read buffer resp.");
  }
