...
class CacheManager(object):
    ...
    def __init__(self, llm_datadist, options) -> None:
        ...
        self._enabled_mem_pool = (
            "llm.MemPoolConfig" in options and len(options["llm.MemPoolConfig"]) > 0
        )
        self._enable_host_mem_pool = (
            "llm.HostMemPoolConfig" in options
            and len(options["llm.HostMemPoolConfig"]) > 0
        )
        self._enable_remote_cache_accessible = (
            options.get("llm.EnableRemoteCacheAccessible", "0") == "1"
        )
    ...
    def allocate_blocks_cache(
        ...
        raise_if_false(
            cache_desc.num_tensors > 0, "num_tensors should be bigger than zero."
        )
        raise_if_false(
            (self._enabled_mem_pool and cache_desc.placement == Placement.DEVICE)
            or (self._enable_host_mem_pool and cache_desc.placement == Placement.HOST),
        ...
        cache = Cache(
            cache_id_and_addr[0], cache_desc, cache_id_and_addr[1][0], self, False, True
        )
    ...
    def copy_cache(
        ...
        raise_if_true(
            src.is_blocks_cache,
            "param check failed, src cache can not be blocks cache.",
        )
        raise_if_true(
            dst.is_blocks_cache,
            "param check failed, dst cache can not be blocks cache.",
        )
        raise_if_false(
            size == -1 or size > 0,
    ...
    def deallocate_blocks_cache(self, cache: Cache) -> None:
        ...
        handle_llm_status(
            ret, "[deallocate_blocks_cache]", f"cache_id = {cache.cache_id}"
        )
        cache._tensor_addrs = []
        cache._valid = False
    ...
    def pull_blocks(
        self,
        src_cache_key: Union[CacheKey, CacheKeyByIdAndIndex, BlocksCacheKey],
        dst_cache: Cache,
        ...
        **kwargs,
        ...
        check_isinstance(
            "src_cache_key",
            src_cache_key,
            [CacheKey, CacheKeyByIdAndIndex, BlocksCacheKey],
        )
        ...
        raise_if_false(
            dst_cache.is_blocks_cache,
            "param check failed, dst_cache should be blocks cache.",
        )
        ...
        raise_if_false(
            ...
            dst_cache.cache_desc.num_tensors,
        )
        ...
        ret = self._llm_datadist.pull_cache_v2(
            dst_cache.cache_id, packed_cache_key, param
        )
    ...
    def register_cache(
        ...
        addrs: List[int],
        cache_keys: Union[Tuple[CacheKey], List[CacheKey]] = (),
        remote_accessible: Optional[bool] = None,
    ) -> Cache:
        """
        Args:
            cache_desc: Cache描述
        ...
        cache = Cache(
            cache_id_and_addr[0], cache_desc, cache_id_and_addr[1][0], self, True, False
        )
        log.info("[register_cache] success, cache_id = %d.", cache.cache_id)
        return cache

    def register_blocks_cache(
        self,
        cache_desc: CacheDesc,
        addrs: List[int],
        ...
        cache = Cache(
            cache_id_and_addr[0], cache_desc, cache_id_and_addr[1][0], self, True, True
        )
    ...
    def remap_registered_memory(self, mem_infos: Union[MemInfo, list[MemInfo]]) -> None:
        ...
        for i, mem_info in enumerate(mem_info_list):
            raise_if_false(
                isinstance(mem_info, MemInfo),
                f"mem_infos[{i}] type only support MemInfo, but got {format(type(mem_info))}.",
            )
            raise_if_false(
                mem_info.mem_type == Memtype.MEM_TYPE_DEVICE,
        ...
        log.info(
            "[remap_registered_memory] start, mem_infos size = %s", len(mem_info_list)
        )
        ...
        handle_llm_status(
            ret, "[remap_registered_memory]", f"mem_info_list = {mem_info_list}"
        )
    ...
    def _verify_caches(src_cache: Cache, dst_cache: Cache, src_to_dst: Dict[int, int]):
        ...
        raise_if_false(
            src_cache.is_blocks_cache,
            "param check failed, src cache should be blocks cache.",
        )
        raise_if_false(
            dst_cache.is_blocks_cache,
            "param check failed, dst cache should be blocks cache.",
        )
        ...
        log.info("src and dst cache block size (bytes):%d", src_block_size)
    ...
    def swap_blocks(
        self, src_cache: Cache, dst_cache: Cache, src_to_dst: Dict[int, int]
    ) -> None:
        """
        交换blocks
        ...
        is_swap_in = (src_placement == Placement.HOST) and (
            dst_placement == Placement.DEVICE
        )
        is_swap_out = (src_placement == Placement.DEVICE) and (
            dst_placement == Placement.HOST
        )
        raise_if_false(
        ...
        default_cache_id = -1
        ret = self._llm_datadist.swap_blocks_v2(
            (default_cache_id, [src_cache.tensor_addrs]),
            (default_cache_id, [dst_cache.tensor_addrs]),
            block_size,
            swap_type,
            self._llm_datadist.dict_to_vector(src_to_dst),
        )
    ...
    def transfer_cache_async(
        ...
        raise_if_true(
            ...
            "src cache not support placement HOST",
        )
        ...
        return transfer_cache_async(
            params,
            layer_synchronizer,
            self._llm_datadist.transfer_cache_v2,
    ...
    def push_cache(
        ...
        check_isinstance(
            "dst_cache_key", dst_cache_key, [CacheKeyByIdAndIndex], allow_none=False
        )
        check_isinstance("src_cache", src_cache, Cache, allow_none=False)
        check_uint32("src_batch_index", src_batch_index)
        raise_if_false(
            not src_cache.is_blocks_cache,
            "param check failed, cache can not be blocks cache.",
        )
        ...
        raise_if_false(
            ...
            tensor_num_per_layer,
            src_cache.cache_desc.num_tensors,
        )
        ...
        if src_layer_range is None:
            src_layer_range = range(
                0, src_cache.cache_desc.num_tensors // tensor_num_per_layer
            )
        if dst_layer_range is None:
            dst_layer_range = range(
                0, src_cache.cache_desc.num_tensors // tensor_num_per_layer
            )
        raise_if_false(
            len(src_layer_range) == len(dst_layer_range),
            "src_layer_range size should be equal to size of dst_layer_range size.",
        )
        check_int64("size", size)
        raise_if_false(
            size == -1,
            "[push_cache] param check failed, size ({0}) is invalid, should be = -1.",
            size,
        )

        log.info(
            "[push_cache] start, cache_id = %d, src_batch_index = %d, dst_cache_key = %s, "
        ...
        for i, src_layer_index in enumerate(list(src_layer_range)):
            ...
            transfer_config = (
                ...
                src_layer_index,
                [],
                dst_cache_key.cluster_id,
                dst_cache_key.cache_id,
                dst_cache_key.batch_index,
    ...
    def push_blocks(
        ...
        raise_if_false(
            src_cache.is_blocks_cache,
            "param check failed, cache should be blocks cache.",
        )
        raise_if_false(len(dst_blocks) > 0, "dst_blocks can not be empty.")
        ...
        log.info(
            "[push_blocks] start, cache_id = %d, dst_cache_key = %s, "
            "src_layer_range = %s, dst_layer_range = %s, tensor_num_per_layer = %d",
            src_cache.cache_id,
            dst_cache_key,
            src_layer_range,
    ...
    def _get_remote_accessible(
        self, func_name, cache_desc: CacheDesc, remote_accessible: Optional[bool] = None
    ):
        if remote_accessible is None:
            inner_remote_accessible = (
                True if cache_desc.placement == Placement.DEVICE else False
            )
            inner_remote_accessible = (
                False if self._is_call_linked else inner_remote_accessible
            )
            log.info(
                "[%s] start, set default remote_accessible = %s when cache placement = %s%s.",
                func_name,
                inner_remote_accessible,
            ...
            log.info(
                "[%s] start, set remote_accessible = %s.",
                func_name,
                inner_remote_accessible,
            )
