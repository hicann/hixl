...
class LLMDataDist(object):
    ...
    def _check_flow_graph_max_size(self, options: Dict[str, str]) -> None:
        ...
        raise_if_false(
            value.isdigit(),
            "ge.flowGraphMemMaxSize must be digit, config value={0}",
            value,
        )
    ...
    def init(self, options: Dict[str, str]) -> None:
        ...
        raise_if_false(
            ...
            "Cannot init multiple LLMDataDists",
        ...
        if self._enable_cache_mgr:
            ...
            ret = self._llm_datadist.initialize_v2(
                self._cluster_id, self._engine_options
            )
            handle_llm_status(
                ret,
                "[LLMDataDist.init]",
            ...
            EngineConfig.gen_cluster_info_if_not_exist(
                self._cluster_id, self._role, self._engine_options
            )
    ...
    def link(
        self, comm_name: str, cluster_rank_info: Dict[int, int], rank_table: str
    ) -> int:
        """
        ...
        raise_if_false(
            len(comm_name) < _MAX_CLUSTER_NAME,
            "comm_name length should be smaller than 128.",
        )
        check_isinstance("cluster_rank_info", cluster_rank_info, dict)
        check_dict("cluster_rank_info", cluster_rank_info, int, int)
        raise_if_false(
            len(cluster_rank_info) <= _MAX_NODE_NUM,
            "cluster_rank_info size can not be greater than 4.",
        )
    ...
    def query_register_mem_status(self, comm_id: int) -> RegisterMemStatus:
        ...
        return (
            int_to_mem_status_dict[status]
            if status in int_to_mem_status_dict
            else RegisterMemStatus.FAILED
        )
    ...
    def _cluster_config(self):
        if self._engine_config is None:
            self._engine_config = EngineConfig.from_engine_options(
                self._role == LLMRole.PROMPT, self._engine_options
            )
    ...
    def check_link_status(self, remote_cluster_id: int):
        ...
        handle_llm_status(
            ret, "[check_link_status]", f"remote_cluster_id is {remote_cluster_id}"
        )
        log.info("[check_link_status] success")

    def link_clusters(
        self, clusters: Union[List[LLMClusterInfo], Tuple[LLMClusterInfo]], timeout=3000
    ):
        self._check_is_inited()
    ...
    def unlink_clusters(
        ...
        if self._enable_cache_mgr:
            ...
            for cluster in clusters:
                check_uint64("remote_cluster_id", cluster.remote_cluster_id)
                cluster_list.append(
                    (cluster.remote_cluster_id, 0, [], cluster.remote_ip_info_list)
                )
            ret, rets = self._llm_datadist.unlink_clusters_v2(
                cluster_list, timeout, force
            )
        else:
            cluster_list = []
            for cluster in clusters:
    ...
    def switch_role(
        self, role: LLMRole, switch_options: Optional[Dict[str, str]] = None
    ):
        self._check_is_inited()
        if self._enable_cache_mgr:
            check_isinstance("role", role, LLMRole)
            ...
            log.info(
                f"[switch_role] [{self._role.name}->{role.name}] start, switch_options = {switch_options}"
            )
    ...
    def _setup_engine_option(self, options: Dict[str, str]) -> None:
        ...
        if self._enable_local_comm_res and "llm.EnableCacheManager" not in options:
            log.info(
                "cache manager is enabled by default when local_comm_res is set in LLMConfig"
            )
            self._engine_options["llm.EnableCacheManager"] = "1"
        if (
            self._enable_local_comm_res
            and "llm.EnableRemoteCacheAccessible" not in options
        ):
            log.info(
                "remote cache accessible is enabled by default when local_comm_res is set in LLMConfig"
            )
            self._engine_options["llm.EnableRemoteCacheAccessible"] = "1"
        self._enable_cache_mgr = (
            "llm.EnableCacheManager" in self._engine_options
            and self._engine_options["llm.EnableCacheManager"] == "1"
        )
