...
from mooncake_sample_common import (
    SEGMENT_SIZE,
    LOCAL_BUFFER,
    HCCS_ALIGNMENT,
    FABRIC_ALIGNMENT,
...
class MooncakeSampleBase:
    ...
    def init_process_group(self):
        ...
        dist.init_process_group(
            backend="gloo", rank=self.config.rank, world_size=self.config.world_size
        )
    ...
    def init_mooncake_dummy_store(self) -> MooncakeDistributedStore:
        store = MooncakeDistributedStore()
        real_client_address = getattr(
            self.args, "real_client_address", "127.0.0.1:50052"
        )
        ...
        store.setup_dummy(mem_pool_size, local_buffer_size, real_client_address)
        logging.info(
            f"Initialized mooncake dummy client, real client at: {real_client_address}"
        )
        return store
    ...
    def create_tensors(self):
        ...
        if use_dummy_mode:
            if self.args.schema.startswith("h"):
                ...
                alloc_size = (
                    (tensor_size + FABRIC_ALIGNMENT - 1)
                    // FABRIC_ALIGNMENT
                    * FABRIC_ALIGNMENT
                )
                ...
                self.tensor = torch.ones(
                    33, 61, 144 * 1024, dtype=torch.int8, pin_memory=True
                ).npu()
            ...
            if self.args.schema.endswith("h"):
                ...
                # Allocate memory from store's memory pool
                alloc_size = (
                    (target_tensor_size + FABRIC_ALIGNMENT - 1)
                    // FABRIC_ALIGNMENT
                    * FABRIC_ALIGNMENT
                )
                tensor_ptr = self.store.alloc_from_mem_pool(alloc_size)
            ...
            if self.args.schema.endswith("h"):
                ...
                self.target_tensor = torch.zeros(
                    33, 61, 144 * 1024, dtype=torch.int8
                ).npu()
    ...
    def register_buffers(self):
        ...
        if use_dummy_mode:
            ...
            remote_addr = (
                (target_data_ptr + HCCS_ALIGNMENT - 1)
                // HCCS_ALIGNMENT
                * HCCS_ALIGNMENT
            )
