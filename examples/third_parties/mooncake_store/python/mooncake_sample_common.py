...
def create_parser(description):
    ...
    parser.add_argument(
        "--device_id", type=int, required=True, help="Device ID (must be provided)"
    )
    parser.add_argument(
        "--rank",
        type=int,
        required=True,
        help="Rank ID (must be provided to sync process group)",
    )
    parser.add_argument(
        "--world_size", type=int, help="World size (optional, default: 1)"
    )
    parser.add_argument(
        "--distributed", action="store_true", help="Enable distributed mode"
    )
...
def validate_schema(args):
    ...
    if args.use_dummy and args.schema not in ["h2h", "d2d"]:
        raise RuntimeError("Only h2h and d2d supported for Dummy/Real Clients now.")
