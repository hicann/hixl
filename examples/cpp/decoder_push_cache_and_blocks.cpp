...
int32_t WaitUnlinkDone(const char *local_ip) {
  int32_t listen_fd = CreateControlServer(local_ip, kDecoderControlPort);
  if (listen_fd < 0) {
    return -1;
  }
  fd_set read_fds;
  ...
  if (ret <= 0) {
    printf("[ERROR] Wait prompt unlink done %s\n", ret == 0 ? "timeout" : "failed");
    CloseSocket(listen_fd);
    return -1;
  }
  ...
  if (nread != static_cast<ssize_t>(sizeof(message)) || message != kUnlinkDoneMessage) {
    printf("[ERROR] Receive prompt unlink done failed\n");
    return -1;
  }
  printf("[INFO] Wait prompt unlink done success\n");
  return 0;
}
}  // namespace
